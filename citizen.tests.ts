...
describe("Temporary file cleanup", () => {
  it("cleans up temporary manifest and contracts directory after success", async () => {
    // Setup
    const tempDir = createIsolatedTestEnvironment(
      resolve(__dirname, "example"),
      isolatedTestEnvPrefix
    ...
    // Verify
    // `.Clarinet.toml.<session-id>`
    const tempManifestPattern = /\.Clarinet\.toml\.\d+$/; // Session ID is a number.
    const filesInTempDir = fs.readdirSync(tempDir);
    const tempManifestsRemaining = filesInTempDir.filter((file) =>
      tempManifestPattern.test(file)
    );
    expect(tempManifestsRemaining).toHaveLength(0);

    const rvContractsDir = join(tempDir, ".rv-contracts");
    expect(existsSync(rvContractsDir)).toBe(false);

    // Teardown
    rmSync(tempDir, { recursive: true, force: true });
  });

  it("cleans up temporary files when contract is not found", async () => {
    // Setup
    const tempDir = createIsolatedTestEnvironment(
      resolve(__dirname, "example"),
      isolatedTestEnvPrefix
    ...
    // Non existent contract. Should throw error.
    await expect(
      issueFirstClassCitizenship(
        tempDir,
        join(tempDir, getManifestFileName(tempDir, "cargo")),
    ...
    // Verify no temporary manifest files remain after error.
    const tempManifestPattern = /\.Clarinet\.toml\.\d+$/;
    const filesInTempDir = fs.readdirSync(tempDir);
    const tempManifestsRemaining = filesInTempDir.filter((file) =>
      tempManifestPattern.test(file)
    );
    ...
    // Verify .rv-contracts directory doesn't exist after error.
    const rvContractsDir = join(tempDir, ".rv-contracts");
    expect(existsSync(rvContractsDir)).toBe(false);
