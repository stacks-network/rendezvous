...
(define-public (send-tokens (transfers (list 5 {
  token: <ft-trait>,
  to: principal,
  amount: uint,
})))
...
(define-private (transfer (one-transfer {
  token: <ft-trait>,
  to: principal,
  amount: uint,
}))
...
(define-private (test-transfer
  ...
  (let (
      (sender-balance-before (unwrap-panic (contract-call? token get-balance tx-sender)))
      (transfer-result (transfer {
        token: token,
        to: to,
        amount: amount,
      }))
      (sender-balance-after (unwrap-panic (contract-call? token get-balance tx-sender)))
    )
    (match transfer-result
      result (ok (is-eq sender-balance-after (- sender-balance-before amount)))
      error (ok false)
    )
  )
)
