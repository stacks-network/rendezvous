...
(define-private (reverse-list (seq (list 127 int)))
...
(define-private (reverse-redx-buffer-buffer
  ...
  (unwrap-panic (as-max-len? (concat item seq) u127))
)

(define-private (reverse-buffer (seq (buff 127)))
...
(define-private (reverse-string1 (seq (string-utf8 127)))
...
(define-private (reverse-string2 (seq (string-ascii 127)))
...
;; #[env(simnet)]
(define-private (test-reverse (seq (list 127 int)))
  (begin
...
(define-private (test-reverse-uint (seq (list 127 uint)))
  (begin
    (asserts! (is-eq seq (reverse-uint (reverse-uint seq))) ERR_ASSERTION_FAILED)
    (ok true)
  )
)

;; #[env(simnet)]
(define-private (test-reverse-buff (seq (buff 127)))
  (begin
    (asserts! (is-eq seq (reverse-buff (reverse-buff seq))) ERR_ASSERTION_FAILED)
    (ok true)
  )
)

;; #[env(simnet)]
(define-private (test-reverse-string (seq (string-utf8 127)))
  (begin
    (asserts! (is-eq seq (reverse-string (reverse-string seq)))
...
;; #[env(simnet)]
(define-private (test-reverse-ascii (seq (string-ascii 127)))
  (begin
    (asserts! (is-eq seq (reverse-ascii (reverse-ascii seq)))
      ERR_ASSERTION_FAILED
    )
    (ok true)
  )
