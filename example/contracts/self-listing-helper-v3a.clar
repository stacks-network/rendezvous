;; The Alex self-listing contract that got exploited in June 2025.

;; Originally copied from:
;; https://explorer.hiro.so/txid/0xa0988bb5f2aa6179e61e7735b91f7276cf70106f05781a0c1c7dee663be5dc7c?chain=mainnet
...
;; Modified the original trait to the one `rendezvous-token` implements.
...
(define-constant tx-version (if is-in-mainnet
	0x00
	0x80
))
...
(define-read-only (calculate-txid
	...
	(let (
		...
		(ok (sha512/256 (concat tx-version
															...
															(concat clarity-version
																(concat (contract-name-length-byte (len contract-name))
																	(concat (string-to-buff contract-name)
																		(concat (contract-code-length-length-bytes (len code-body))
																			code-body
																		))
																))
														))
...
(define-read-only (is-dao-or-extension)
	(ok (asserts!
		(or (is-eq tx-sender 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.executor-dao) (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.executor-dao
...
(define-read-only (get-approved-token-x-or-default (token-x principal))
	(default-to {
		...
		(map-get? approved-token-x token-x)
	)
)
...
(define-read-only (get-lock-period)
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		get-lock-period
	)
)

(define-read-only (get-locked-liquidity-or-default
		(owner principal)
		(pool-id uint)
	)
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		get-locked-liquidity-or-default owner pool-id
	)
)

(define-read-only (get-locked-liquidity-for-pool-or-default (pool-id uint))
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		get-locked-liquidity-for-pool-or-default pool-id
	)
)

(define-read-only (get-burnt-liquidity-or-default (pool-id uint))
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		get-burnt-liquidity-or-default pool-id
	)
)

(define-read-only (get-wrapped-token-contract-code (token principal))
	(let (
			(token-str (unwrap-panic (as-max-len? (principal-to-string token) u100)))
			(template (var-get wrapped-token-template))
		)
		(get result
			(fold join-template-parts-iter template {
				token: token-str,
				result: "",
			})
		)
...
(define-read-only (principal-to-string (p principal))
	(let (
			(destructed (match (principal-destruct? p)
				ok-value ok-value
				err-value err-value
			))
...
(define-read-only (verify-deploy (verify-params {
	nonce: (buff 8),
	fee-rate: (buff 8),
	signature: (buff 65),
	contract: principal,
	...
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.clarity-stacks
		was-tx-mined-compact
		(try! (calculate-txid (get nonce verify-params) (get fee-rate verify-params)
			(get signature verify-params) (get contract verify-params)
			(contract-call?
				'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.clarity-stacks-helper
				string-ascii-to-buffer
				(get-wrapped-token-contract-code (get token-y verify-params))
			)))
		(get proof verify-params) (get tx-block-height verify-params)
		(get block-header-without-signer-signatures verify-params)
	)
)

;; Public functions
(define-public (create (request-details {
	...
	(let ((token-y-trait (get token-y-trait request-details)))
		...
		(asserts!
			(< u0
				(contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-vault-v2-01
					get-reserve (contract-of token-y-trait)
				))
...
(define-public (create2
	...
	(let ((token-y-trait (get token-y-trait request-details)))
		(asserts! (is-eq (contract-of token-y-trait) (get contract verify-params))
			err-token-not-approved
		)
		(try! (pre-check request-details))
		(try! (verify-deploy verify-params))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-vault-v2-01
			set-approved-token (contract-of token-y-trait) true
		))
		(map-set wrap-token-map (get token-y verify-params)
...
(define-public (lock-liquidity
		(amount uint)
		(pool-id uint)
	)
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		lock-liquidity amount pool-id
	)
)

(define-public (burn-liquidity
		(amount uint)
		(pool-id uint)
	)
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		burn-liquidity amount pool-id
	)
)

(define-public (claim-liquidity (pool-id uint))
	(contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.liquidity-locker
		claim-liquidity pool-id
	)
)

;; Governance calls
(define-public (approve-token-x
		(token principal)
		(approved bool)
		(min-x uint)
	)
	(begin
		...
		(ok (map-set approved-token-x token {
			approved: approved,
			min-x: min-x,
		}))
	)
...
;; Private functions
(define-private (pre-check (request-details {
	token-x-trait: <ft-trait>,
	token-y-trait: <ft-trait>,
	factor: uint,
	bal-x: uint,
	...
	(let (
		...
		(asserts! (get approved token-x-details) (err u98765432)
			;; err-token-not-approved
		)
		(asserts! (>= (get bal-x request-details) (get min-x token-x-details))
			err-insufficient-balance
		)
		(asserts!
			(and
				(is-none (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
					get-pool-exists (contract-of token-x-trait) (contract-of token-y-trait)
					(get factor request-details)
				))
				(is-none (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
					get-pool-exists (contract-of token-y-trait) (contract-of token-x-trait)
					(get factor request-details)
				))
			)
			err-pool-exists
		...
		(asserts!
			(or
				(is-eq (get lock request-details) LOCK)
				(is-eq (get lock request-details) BURN)
				(is-eq (get lock request-details) NONE)
			)
...
(define-private (post-check (request-details {
	token-x-trait: <ft-trait>,
	token-y-trait: <ft-trait>,
	factor: uint,
	bal-x: uint,
	...
	(let (
			...
			(supply (get supply
				(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
					create-pool token-x-trait token-y-trait factor tx-sender
					(get bal-x request-details) (get bal-y request-details)
				))
			))
			(pool-id (get pool-id
				(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
					get-pool-details token-x token-y factor
				))
			))
		)
		...
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-fee-rate-x token-x token-y factor (get fee-rate-x request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-fee-rate-y token-x token-y factor (get fee-rate-y request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-max-in-ratio token-x token-y factor (get max-in-ratio request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-max-out-ratio token-x token-y factor (get max-out-ratio request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-threshold-x token-x token-y factor (get threshold-x request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-threshold-y token-x token-y factor (get threshold-y request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-oracle-enabled token-x token-y factor
			(get oracle-enabled request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-oracle-average token-x token-y factor
			(get oracle-average request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01
			set-start-block token-x token-y factor (get start-block request-details)
		))
		(try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-registry-v2-01
			set-fee-rebate token-x token-y factor (var-get fee-rebate)
		))
		(ok pool-id)
	)
)
...
(define-private (c32-to-string-iter
		(idx int)
		(it {
			s: (string-ascii 39),
			r: uint,
...
(define-private (trim-leading-0-iter
		(idx int)
		(it (string-ascii 39))
	)
...
(define-private (append-leading-0-iter
		(idx int)
		(it {
			hash-bytes: (buff 24),
...
(define-private (append-leading-0
		(hash-bytes (buff 24))
		(s (string-ascii 39))
	)
	(get address
...
(define-private (join-template-parts-iter
		(part (string-ascii 5000))
		(state {
			token: (string-ascii 100),
			result: (string-ascii 10000),
		})
	)
	{
		token: (get token state),
		result: (unwrap-panic (as-max-len?
			(concat (get result state)
				(if (is-eq (len (get result state)) u0)
					part
					(concat (get token state) part)
				))
			u10000
		)),
	}
