...
(define-public (create-new-shipment
    (starting-location (string-ascii 25))
    (receiver principal)
  )
  (let ((new-shipment-id (+ (var-get last-shipment-id) u1)))
...
;; #[env(simnet)]
(define-private (test-create-new-shipment
    (starting-location (string-ascii 25))
    (receiver principal)
  )
  (let (
      ...
      (result (unwrap! (create-new-shipment starting-location receiver)
...
(define-private (test-get-last-shipment-id
  ...
  (let ((shipment-id-before (get-last-shipment-id)))
    (unwrap! (create-new-shipment starting-location receiver)
      ERR_CONTRACT_CALL_FAILED
    )
    ;; Verify the last shipment ID is incremented by 1.
    (asserts! (is-eq (get-last-shipment-id) (+ u1 shipment-id-before))
...
;; #[env(simnet)]
(define-private (test-update-shipment
    (starting-location (string-ascii 25))
    (receiver principal)
    (new-location (string-ascii 25))
  )
  (let (
      (create-result (unwrap! (create-new-shipment starting-location receiver)
        ERR_CONTRACT_CALL_FAILED
      ))
      (shipment-id (get-last-shipment-id))
      (update-result (unwrap! (update-shipment shipment-id new-location)
        ERR_CONTRACT_CALL_FAILED
      ))
      (updated-shipment (default-to {
    ...
    (asserts! (is-eq (get location updated-shipment) new-location)
      ERR_ASSERTION_FAILED
    )
    (ok true)
  )
...
;; #[env(simnet)]
(define-private (test-get-shipment
    (starting-location (string-ascii 25))
    (receiver principal)
  )
  (let (
      ...
      (shipment-id (get-last-shipment-id))
      (retrieved-shipment (unwrap! (get-shipment-optional shipment-id) ERR_SHIPMENT_NOT_FOUND))
    )
    ;; Verify the location is correct.
    (asserts! (is-eq (get location retrieved-shipment) starting-location)
      ERR_ASSERTION_FAILED
    )
    (ok true)
  )
